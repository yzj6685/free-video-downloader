...
- `POST /api/ai/analyze`：提取平台字幕，调用 DeepSeek 生成摘要、大纲、核心知识点、推荐问题和转录片段。
- `POST /api/ai/chat`：基于当前 `analysis_id` 对视频内容追问。

后端新增模块：

- `app/services/subtitle_service.py`：读取 yt-dlp 元数据中的手工字幕和自动字幕，中文优先，其次英文，再兜底任意可用字幕；支持 `vtt`、`srt`、`json3`。
- `app/services/deepseek_client.py`：封装 DeepSeek Chat Completions 请求，使用 JSON 输出生成结构化学习笔记。
- `app/services/ai_analysis_service.py`：清洗字幕、长文本分块、结果合成、内存 TTL 缓存和问答上下文组织。
- `app/routers/ai_analysis.py`：挂载 `/api/ai/analyze`、`/api/ai/analyze-stream` 与 `/api/ai/chat`。
...
- AI 返回格式异常：重试一次，仍失败则返回稳定的 502 错误。
- 问答缓存过期：返回 404，提示重新分析视频。

前端交互：

- 解析成功后在结果卡片内提供“AI 分析”按钮，并在下方展示 AI 学习笔记面板。
- 面板包含总结摘要、字幕文本、思维导图、AI 问答四个标签页；总结页内同时展示摘要、大纲、核心知识点和推荐追问。
- 转录文本支持复制全文；问答保留当前页面会话历史。
